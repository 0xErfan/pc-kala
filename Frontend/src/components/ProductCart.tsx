...
const ProductCart = ({ price, finalPrice, count, title, src, id }: ProductCartProps) => {
    ...
    return (
        ...
        <tr className="border-y border-gold/40 text-[13px] ch:border-l ch:border-dark-gold ch:last:border-l-none">
            <td className="flex ch:border-l ch:border-dark-gold">
                ...
                <Link to={`/product/${id}`} className="flex-[8] hover:text-blue-white transition-all duration-200 cursor-pointer h-full m-auto px-2 last:border-none">{title}</Link>
            </td>
