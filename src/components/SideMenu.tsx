...
const SideMenu = ({ dataToShow, changeTypeFn }: SideMenuProps) => {
    ...
    return (
        <>
            ...
            <div className={`fixed h-screen bg-primary-black overflow-y-auto top-0 bottom-0 transition-all duration-300 ${isMenuShown ? ` shadow-regular ${dataToShow == "sideMenu" ? "right-0" : "left-0"}` : ` ${dataToShow == "sideMenu" ? "-right-[290px]" : "-left-[290px]"}`} z-40`}>
                {
                    dataToShow !== "sideMenu"
                        ?
