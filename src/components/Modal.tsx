...
const Modal = () => {
    ...
    return (
        <section onClick={e => String(e.target?.className ?? '').includes('modalContainer') && notifyUserToClickButtons()} className={`fixed modalContainer duration-200 transition-all ${isShown ? 'opacity-100 visible' : 'opacity-0 invisible'} w-full z-[1000] h-full flex-center inset-0 m-auto bg-transparent`}>
